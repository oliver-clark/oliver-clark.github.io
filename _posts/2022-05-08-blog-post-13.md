...
This post is a work in progress. Below I have pasted the .sh script I wrote today to create PRS.
...
- **Arg #1**: A list of LD pruned SNPs from the LD_pruning.sh script
 ...
 - **Arg #2**: A table of SNPs and their associated GWAS data. This was created in previous posting
  ...
  - Importantly, there should be the following columns, which will also be added as 
    ...
    - **Arg #5**: column # for rsid
    - **Arg #6**: column # for "STRONGEST SNP-RISK ALLELE"
      - This may be in the format "rsid-allele"
    - **Arg #7**: column # for the odds ratio
    - **Arg #8**: column # for the Variant ID, in the format chr_pos
- **Arg #9**: PMBB ID file to keep. E.g. this is a curated .txt file of European PMBB ID numbers
- **Arg #10**: PMBB ID relateds file to remove
...
# $8 column # for chr_pos
# $9 PMBB ID file to keep
# $10 PMBB ID relateds file to remove

# Example script call:
# bsub -e build_PRS.e -o build_PRS.o ../OC_build_PRS.sh input.prune.in ../master_snplist_lung_cancer_all.txt 12 13 23 22 32 14 PMBBID.txt relateds.txt
...
# First, replace third column : with _

# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr1_temp.recode.vcf > chr1_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr2_temp.recode.vcf > chr2_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr3_temp.recode.vcf > chr3_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr4_temp.recode.vcf > chr4_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr5_temp.recode.vcf > chr5_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr6_temp.recode.vcf > chr6_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr7_temp.recode.vcf > chr7_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr8_temp.recode.vcf > chr8_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr9_temp.recode.vcf > chr9_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr10_temp.recode.vcf > chr10_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr11_temp.recode.vcf > chr11_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr12_temp.recode.vcf > chr12_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr13_temp.recode.vcf > chr13_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr14_temp.recode.vcf > chr14_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr15_temp.recode.vcf > chr15_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr16_temp.recode.vcf > chr16_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr17_temp.recode.vcf > chr17_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr18_temp.recode.vcf > chr18_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr19_temp.recode.vcf > chr19_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr20_temp.recode.vcf > chr20_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr21_temp.recode.vcf > chr21_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chr22_temp.recode.vcf > chr22_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub(":", "_", $3) ; gsub(":", "_", $3) ; print }' chrX_temp.recode.vcf > chrX_temp.recode.vcf


# Next, remove third column "chr"
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr1_temp.recode.vcf > chr1_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr2_temp.recode.vcf > chr2_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr3_temp.recode.vcf > chr3_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr4_temp.recode.vcf > chr4_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr5_temp.recode.vcf > chr5_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr6_temp.recode.vcf > chr6_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr7_temp.recode.vcf > chr7_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr8_temp.recode.vcf > chr8_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr9_temp.recode.vcf > chr9_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr10_temp.recode.vcf > chr10_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr11_temp.recode.vcf > chr11_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr12_temp.recode.vcf > chr12_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr13_temp.recode.vcf > chr13_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr14_temp.recode.vcf > chr14_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr15_temp.recode.vcf > chr15_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr16_temp.recode.vcf > chr16_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr17_temp.recode.vcf > chr17_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr18_temp.recode.vcf > chr18_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr19_temp.recode.vcf > chr19_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr20_temp.recode.vcf > chr20_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr21_temp.recode.vcf > chr21_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chr22_temp.recode.vcf > chr22_temp.recode.vcf
# awk -F'\t' -vOFS='\t' '{ gsub("chr", "", $3) ; gsub("chr", "", $3) ; print }' chrX_temp.recode.vcf > chrX_temp.recode.vcf
...
# Create list of merged bed, bim, and fam files
# filelist_temp.txt

# Merge all .bed files into a single file
# plink --merge-list filelist_temp.txt --allow-no-sex --make-bed --out merged_temp

# Keep PMBB IDs of interest, remove relateds, and create file merged_subset_temp
# plink --bfile merged_temp --keep $8 --remove $9 --make-bed --out merged_subset_temp
...
# Save them to temp_snp_columns.txt file
awk -F '\t' -v FS='\t' -v OFS='\t' -v chr="$3" -v pos="$4" -v chr_pos="$8" -v rsid="$5" -v oddsratio="$7" '{print $chr, $pos, $chr_pos, $rsid, $oddsratio}' $2 > temp_snp_columns.txt

# Combine the above to text files
# We not have a tab-delimited file called temp_snps.txt which contains columns for chr, pos, rsid, odds ratio, and risk allele
paste -d "\t" temp_snp_columns.txt temp_risk_alleles.txt > temp_snps.txt

sort -k3 -o temp_snps.txt temp_snps.txt

# Get a list of the common SNPs within our LD_pruning file and master SNPs file
# Save common list as common_snps_temp.txt
comm -12 <(awk '{ print $3 }' temp_snps.txt) <(awk '{ print $1 }' $1) > common_snps_temp.txt

# Intersect our common list (common_snps_temp.txt) with the master list of SNPs (temp_snps.txt)
# Save the final subsetted SNP file as snp_subset_temp.txt
awk 'FNR==NR {a[$1]; next}; $3 in a' common_snps_temp.txt temp_snps.txt > snp_subset_temp.txt
...
rm common_snps_temp.txt
rm temp_snps.txt

# Use temp_snps file of SNPs with odds ratio data and merged_temp file of genotype information to create PRS
#plink --bfile merged_temp --score snp_subset_temp.txt 4 6 5 header --out PRS_output
```
