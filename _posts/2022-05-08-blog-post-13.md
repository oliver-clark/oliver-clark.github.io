...
- **Arg #1**: A list of LD pruned SNPs from the LD_pruning.sh script
 ...
 - **Arg #2**: A table of SNPs and their associated GWAS data. This was created in previous posting
  ...
  - Importantly, there should be the following columns, which will also be added as 
    ...
    - **Arg #5**: column # for "STRONGEST SNP-RISK ALLELE"
      - This may be in the format "rsid-allele"
    - **Arg #6**: column # for the odds ratio
    - **Arg #7**: column # for the Variant ID, in the format chr_pos
...
# Example script call:
# bsub -e build_PRS.e -o build_PRS.o ../OC_build_PRS.sh input.prune.in ../master_snplist_lung_cancer_all.txt 12 13 14 22 32 PMBBID.txt relateds.txt PMBB_subset_PRS
...
# Columns are
	...
	# 4 - alt
	# 5 - chr_pos_ref_alt
awk  'FNR==NR{a[$1,$2,$5]; next} ($1,$4,$5) in a{print $1 "\t" $4 "\t" $6 "\t" $5 "\t" $1 "_" $4 "\t" $1 "_" $4 "_" $6 "_" $5 }' snplist_temp4.txt merged_subset_temp.bim > snplist_temp6.txt
sort -o snplist_temp6.txt snplist_temp6.txt
sort -o snplist_temp4.txt snplist_temp4.txt

# After sorting, get the matching columns in the snp_subset_temp file, in the correct order. This outputs file in the format chr_pos
comm -12 <(awk '{ print $1 "_" $2 }' snplist_temp4.txt) <(awk '{ print $1 "_" $2 }' snplist_temp6.txt) > snplist_temp7.txt
...
paste snplist_temp8.txt snplist_temp6.txt > snplist_temp9.txt

# Use temp_snps file of SNPs with odds ratio data and merged_subset_temp file of genotype information to create PRS
plink --bfile merged_subset_temp --score snplist_temp9.txt 11 5 4 header --out ${10}
